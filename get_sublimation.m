    ...
    for l = 1:3 % lake counter
        for season = 1:2 % 1 = winter; 2 = summer
            ...
            for itime = 1:nrows
                ...
                % Lake Fryxell snow threshold
                if Fryxell_albedo == 1 && l == 3 && albedo.albedo(floor(itime/24)+1) > 1    % snow cover
                    Qe(itime) = NaN;
                    water_mm(itime) = 0;
                else                                                    % no snow cover
        ...
        for day = 1:length(Date)
            ...
            elseif month(Date(day)) < 2 || month(Date(day)) > 11
                sublimation(day,l) = water_daily(day,2);
            else
