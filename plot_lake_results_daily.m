...
% Zero Line
line([datetime(1990,1,1), datetime(2020,1,1)],[0,0], 'Color','k', 'LineWidth', 1, 'HandleVisibility','off')

xticks([datetime(1990,1,1):calyears(1):datetime(2020,1,1)]);
...
text(datetime(2004,1,1), 3.5, 'Lake Bonney', 'FontWeight', 'bold', 'FontSize',14);
...
ylim([-1 1.5])
yticklabels('auto')
text(datetime(2004,1,1), 1.25,'Lake Hoare', 'FontWeight', 'bold', 'FontSize',14);
...
ylim([-1 1.5])
yticklabels('auto')
text(datetime(2004,1,1), 1.25,'Lake Fryxell', 'FontWeight', 'bold', 'FontSize',14);
...
set(ha(1:3),'Xlim', [datetime(1996,1,1) datetime(2014,1,1)]);
% set(ha(1:3),'Xlim', [datetime(1990,1,1) datetime(2020,1,1)]);
...
yearStart = datenum([year-1 4 1]);
yearEnd = datenum([year 3 31]);

metdates = [datenum(1994,12,1):datenum(2013,2,1)];

start = find(metdates == yearStart);
stop = find(metdates == yearEnd);

inputTimes = (yearStart: 1.0/24.0: yearEnd).';
...
xlim([datetime(year-1,4,1) datetime(year,4,1)])
xticks([datetime(year-1,4,1):calmonths(1):datetime(year,4,1)]);
xtickangle(45)
dateformat = 'mmm yyyy';
datetick('x',dateformat, 'KeepTicks')
